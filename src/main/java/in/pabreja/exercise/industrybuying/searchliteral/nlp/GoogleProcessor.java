...
public class GoogleProcessor implements NLPProcessor {
    ...
    public Map<NLPEngineType, Map<String, Double>>  process(List<IbMessageData> data) {
        ...
        for(IbMessageData m : data){
            if("spam".equalsIgnoreCase(m.getgTruthVal())){
                spamCount++;
                avgSpam = avgSpam + Double.parseDouble(m.getIbmSpamProbVal());
            }
            else if("not-spam".equalsIgnoreCase(m.getgTruthVal())){
                notSpamCount++;
